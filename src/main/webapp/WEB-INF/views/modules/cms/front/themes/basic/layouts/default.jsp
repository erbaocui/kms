...
<body>
	<div class="navbar navbar-fixed-top" style="position:static;margin-bottom:10px;">
      <div class="navbar-inner">
        <div class="container">
			<a class="brand" href="#">中怡知识管理系统</a>
          <c:choose>
			<c:when test="${not empty site.logo}">
			<%--	<img alt="管理系统" src="${site.logo}" class="container" onclick="location='${ctx}/index-${site.id}${fns:getUrlSuffix()}'">--%>
			</c:when>
			<c:otherwise><%--<a class="brand" href="${ctx}/index-${site.id}${fns:getUrlSuffix()}">文档管理系统</a>--%></c:otherwise>
		</c:choose>
